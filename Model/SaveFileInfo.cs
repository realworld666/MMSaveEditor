...
public class SaveFileInfo
{
  ...
  public static SaveFileInfo Create(string name, bool isAutoSave)
  {
    ...
    saveFileInfo.gameInfo.gameTime = instance.time.now;
    saveFileInfo.gameInfo.teamName = instance.player.team.name;
    saveFileInfo.gameInfo.teamLogoID = instance.player.team.teamID;
    saveFileInfo.gameInfo.teamColor = App.instance.teamColorManager.GetColor(instance.player.team.colorID);
  ...
  {
    for (int index = 0; index < this.ownedDLCInfo.Count; ++index)
    {
      Dlc dlc = this.ownedDLCInfo[index];
      if (dlc.isOwned && !App.instance.dlcManager.IsDlcInstalled(dlc.appId))
        return false;
    }
    return true;
  }
  ...
  public class SaveInfo
  {
    ...
    public string sanitisedName
    {
      get
      {
        return SaveSystem.SanitiseFileNameCharacters(this.sanitisedName);
      }
    }
